...
public final class NexusProtocolGameTests {
    ...
    public static void countAndJobDeltasPreserveComponentsAndRejectStaleBaselines(GameTestHelper h) {
        ...
        h.assertTrue(delta.counts().size()==1&&delta.counts().getFirst().slot()==17&&delta.jobChanges().isEmpty()&&delta.jobOrder()==null&&delta.error()==null,"Count delta carries only one slot/count/flag update");
        ...
        var reconstructed=NetworkPackets.reconstruct(first,decoded);h.assertTrue(reconstructed!=null&&reconstructed.entries().get(17).count()==9&&ItemStack.isSameItemSameComponents(reconstructed.entries().get(17).stack(),originalRows.get(17).stack()),"Wire round-trip retains original exact named-item identity");
        ...
        h.assertTrue(jobDelta.counts().isEmpty()&&jobDelta.jobChanges().size()==1&&jobDelta.jobChanges().getFirst().target().isEmpty(),"Progress reuses its known UUID target and sends no storage or item identity");
        ...
        var completed=NetworkPackets.reconstruct(next,jobCopy);h.assertTrue(completed!=null&&completed.jobs().getFirst().completed()==3&&completed.jobs().getFirst().target().is(Items.OAK_PLANKS),"Progress codec restores the known target");
    ...
    public static void actualMenuCoalescesScrollingAndSendsNothingForIdleShelfPolling(GameTestHelper h) {
        ...
        h.startSequence().thenWaitUntil(()->{
            ...
            f.menu().sendPage();h.assertTrue(f.packets().size()==1&&f.packets().getFirst() instanceof NetworkPackets.Page,"Opening sends one complete bounded window");
            ...
            h.runAfterDelay(45,()->{
                ...
                f.menu().sendPage();h.assertTrue(f.packets().size()==1&&f.packets().getFirst() instanceof NetworkPackets.Delta delta&&delta.counts().size()==1,"Real indexed count mutation sends exactly one count delta");
                ...
                h.assertTrue(ItemStack.isSameItemSameComponents(f.menu().entries.getFirst().stack(),top.entries().get(9).stack()),"Scrolling one row advances nine entries, not a page");
                f.packets().clear();search(f.menu(),999,"Entry 079");f.menu().sendPage();
                h.assertTrue(f.menu().scrollRow==0&&f.menu().totalEntries==1&&f.menu().entries.getFirst().stack().getHoverName().getString().equals("Entry 079"),"Search clamps the old scroll position and returns the actual named resource");
                ...
                f.menu().sendPage();h.assertTrue(f.menu().currentPage().request()==10&&f.menu().entries.getFirst().stack().getHoverName().getString().equals("Entry 078"),"A stale request cannot replace the latest query even at the same row");h.succeed();
            });
        });
    }
    ...
    public static void stackRequestsCraftAdditionalOutputsAndCancelOnlyTheOwnedJob(GameTestHelper h) {
        ...
        h.startSequence().thenWaitUntil(()->{
            ...
            h.assertTrue(own.size()==1&&own.getFirst().count()==64&&own.getFirst().total()==16,"Shift-right uses the server's 64-item maximum and plans sixteen real recipes");
            UUID first=own.getFirst().id();h.assertTrue(f.menu().network().snapshot().getOrDefault(planks,0L)==7&&f.menu().network().snapshot().getOrDefault(logs,0L)==48,"Seven existing outputs stay in storage while sixteen logs enter physical escrow");

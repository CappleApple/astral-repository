...
public final class RunePresetFiles {
    ...
    public static RunePreset decode(String text,HolderLookup.Provider registries)throws Exception{if(text.length()>196608)throw new IllegalArgumentException("Preset too large");JsonObject o=JsonParser.parseString(text).getAsJsonObject();List<RuneDesign.Icon> icons=new ArrayList<>();for(var value:o.getAsJsonArray("icons")){var i=value.getAsJsonObject();icons.add(new RuneDesign.Icon(Identifier.parse(i.get("item").getAsString()),i.get("x").getAsFloat(),i.get("y").getAsFloat(),i.get("scale").getAsFloat(),i.get("rotation").getAsFloat()));}FilterRules filter=new FilterRules();filter.load(TagParser.parseCompoundFully(o.get("filter").getAsString()),registries);return new RunePreset(UUID.fromString(o.get("id").getAsString()),o.get("name").getAsString(),RuneLayer.Mode.valueOf(o.get("mode").getAsString()),decodeDesign(o,icons),filter.save(registries),o.get("priority").getAsInt(),o.get("enabled").getAsBoolean(),o.has("cadence")?RuneCadence.load(TagParser.parseCompoundFully(o.get("cadence").getAsString())):RuneCadence.DEFAULT);}

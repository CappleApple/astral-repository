...
public final class WandScreen extends Screen implements GhostIngredientScreen {
    ...
    private static final Identifier PANEL=Identifier.fromNamespaceAndPath("astral_repository","textures/gui/rune_settings.png");
    private static final Identifier BUTTON=Identifier.fromNamespaceAndPath("astral_repository","rune_button"),HOVER=Identifier.fromNamespaceAndPath("astral_repository","rune_button_hovered");
    ...
    public static void receive(WandPackets.Page page){var mc=Minecraft.getInstance();if(page.open()){mc.gui.setScreen(new WandScreen(page.session(),page.data().contains("EditPreset")?RunePreset.load(page.data().getCompoundOrEmpty("EditPreset"),mc.level.registryAccess()):null));return;}if(!(mc.gui.screen() instanceof WandScreen screen)||!screen.session.equals(page.session()))return;
        if(page.data().contains("PackPreset")){try{var p=RunePreset.load(page.data().getCompoundOrEmpty("PackPreset"),mc.level.registryAccess());screen.acceptPack(p);}catch(RuntimeException failure){screen.error="Invalid pack preset";}return;}
        if(!page.data().getStringOr("Error", "").isEmpty())screen.error=page.data().getStringOr("Error", "");
    }
    ...
    private void send(WandPackets.Op op,UUID id,CompoundTag data){net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new WandPackets.Action(session,op,id==null?new UUID(0,0):id,data));}
    ...
    private void rebuild(){if(width>0&&height>0)rebuildWidgets();}
    ...
    private void use(){if(draft!=null){flush();send(WandPackets.Op.SELECT,current,new CompoundTag());minecraft.gui.setScreen(null);}}
    ...
    @Override protected void init(){left=(width-W)/2;top=(height-H)/2;
        ...
        if(behavior){
            ...
            button(278,9,134,18,"Color",()->{colorPicker=new RuneColorPicker(brush,c->{brush=c;erase=false;selectedIcon=-1;},()->colorPicker=null);colorPicker.init(width,height);});
    ...
    private void refresh(){
        if(behavior){
            ...
            var resources=minecraft.player.getCapability(net.neoforged.neoforge.capabilities.Capabilities.Item.ENTITY_AUTOMATION,null); var handler=resources==null?null:com.cappleapple.astralrepository.port.storage.IItemHandler.of(resources);
            if(handler==null)handler=com.cappleapple.astralrepository.port.storage.IItemHandler.of(net.neoforged.neoforge.transfer.item.PlayerInventoryWrapper.of(minecraft.player));
    ...
    private Button button(int x,int y,int w,int h,String text,Runnable action){return addRenderableWidget(new Button(left+x,top+y,w,h,Component.literal(text),b->action.run(),supplier -> supplier.get()){@Override protected void extractContents(GuiGraphicsExtractor g,int mx,int my,float partial){g.blitSprite(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED,isHovered()?HOVER:BUTTON,getX(),getY(),getWidth(),getHeight());GuiText.centeredText(g,font,getMessage(),getX()+getWidth()/2,getY()+(getHeight()-8)/2,0xd9defb);}});}
    private EditBox edit(int x,int y,int w,int h,String value,java.util.function.Consumer<String> responder){EditBox e=new AstralEditBox(font,left+x,top+y,w,h,Component.empty());e.setBordered(false);e.setMaxLength(64);e.setValue(value);e.setTextColor(GuiText.opaque(0xd9defb));e.setResponder(responder);return addRenderableWidget(e);}
    ...
    @Override public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent event, boolean doubleClick){double x=event.x(), y=event.y(); int button=event.button();
        if(colorPicker!=null){colorPicker.mouseClicked(new net.minecraft.client.input.MouseButtonEvent(x,y,new net.minecraft.client.input.MouseButtonInfo(button,AstralInput.modifiers())), false);return true;}
        if(!editing){int i=libraryCell(x,y);if(i>=0){var entries=new ArrayList<>(library.values());if(i==entries.size()&&entries.size()<64)newPreset();else if(i<entries.size()){if(button==1){editing=true;load(entries.get(i));}else if(button==0){load(entries.get(i));use();}}}return i>=0||super.mouseClicked(new net.minecraft.client.input.MouseButtonEvent(x,y,new net.minecraft.client.input.MouseButtonInfo(button,AstralInput.modifiers())), false);}
    ...
    @Override public boolean keyPressed(net.minecraft.client.input.KeyEvent event){int key=event.key(), scan=event.scancode(), modifiers=event.modifiers();if(colorPicker!=null){if(key==com.mojang.blaze3d.platform.InputConstants.KEY_ESCAPE)colorPicker.onClose();else colorPicker.keyPressed(new net.minecraft.client.input.KeyEvent(key,scan,modifiers));return true;}if(editing&&AstralInput.hasControlDown()&&key==com.mojang.blaze3d.platform.InputConstants.KEY_Z&&!undo.isEmpty()){var art=undo.removeLast();pixels=art.argbPixels();icons.clear();icons.addAll(art.icons());selectedIcon=-1;changed();rebuild();return true;}return super.keyPressed(new net.minecraft.client.input.KeyEvent(key,scan,modifiers));}
    @Override public boolean charTyped(net.minecraft.client.input.CharacterEvent event){int c=event.codepoint(), modifiers=AstralInput.modifiers();return colorPicker!=null?colorPicker.charTyped(new net.minecraft.client.input.CharacterEvent(c)):super.charTyped(new net.minecraft.client.input.CharacterEvent(c));}
    @Override public boolean mouseDragged(net.minecraft.client.input.MouseButtonEvent event, double dx, double dy){double x=event.x(), y=event.y(); int button=event.button();if(colorPicker!=null){colorPicker.mouseDragged(new net.minecraft.client.input.MouseButtonEvent(x,y,new net.minecraft.client.input.MouseButtonInfo(button,AstralInput.modifiers())), dx, dy);return true;}if(dragging){paint(x,y,button);return true;}return super.mouseDragged(new net.minecraft.client.input.MouseButtonEvent(x,y,new net.minecraft.client.input.MouseButtonInfo(button,AstralInput.modifiers())), dx, dy);}
    @Override public boolean mouseReleased(net.minecraft.client.input.MouseButtonEvent event){double x=event.x(), y=event.y(); int button=event.button();if(colorPicker!=null){colorPicker.mouseReleased(new net.minecraft.client.input.MouseButtonEvent(x,y,new net.minecraft.client.input.MouseButtonInfo(button,AstralInput.modifiers())));return true;}if(dragging){dragging=false;flush();return true;}return super.mouseReleased(new net.minecraft.client.input.MouseButtonEvent(x,y,new net.minecraft.client.input.MouseButtonInfo(button,AstralInput.modifiers())));}
...
if(in(x,y,8,34,98,192)){listOffset=Math.clamp(listOffset-(int)Math.signum(dy),0,Math.max(0,library.size()-8));return true;}if(in(x,y,278,77,134,38)){resultOffset=Math.clamp(resultOffset-(int)Math.signum(dy)*6,0,Math.max(0,results.size()-12));return true;}if(draft!=null&&selectedIcon>=0&&in(x,y,120,80,144,144)){var icon=icons.get(selectedIcon);transform(AstralInput.hasShiftDown()?2:3,Float.toString((AstralInput.hasShiftDown()?icon.scale():icon.rotation())+(float)Math.signum(dy)*NumericScroll.step()));rebuild();return true;}return super.mouseScrolled(x,y,dx,dy);}
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float partial){AstralInterfaceRenderer.blit(g,PANEL,left,top,W,H);GuiText.text(g,font,"Rune library",left+9,top+13,0xe1d4ff,false);for(var child:children())if(child instanceof AbstractWidget widget)widget.extractRenderState(g,mx,my,partial);
        if(!editing){
            ...
            for(int i=listOffset*8;i<Math.min(count,listOffset*8+32);i++){int k=i-listOffset*8,x=left+20+k%8*48,y=top+40+k/8*48;RuneUi.slot(g,x,y,44,i==hover);
                if(i==entries.size())GuiText.centeredText(g,font,"+",x+22,y+18,0xd9defb);else RuneDesignRenderer.draw(g,entries.get(i).design(),resolution(),x+5,y+5,34);
            }
            if(hover>=0&&hover<entries.size())g.setTooltipForNextFrame(font,Component.literal(entries.get(hover).name()),mx,my);
            if(!error.isEmpty())GuiText.text(g,font,error,left+10,top+242,0xff8899,false);return;
        }
        ...
        if(draft!=null){g.fill(left+119,top+79,left+265,top+225,0xff7777aa);for(int y=0;y<18;y++)for(int x=0;x<18;x++)g.fill(left+120+x*8,top+80+y*8,left+128+x*8,top+88+y*8,(x+y)%2==0?0xff14182e:0xff202440);
            RuneDesignRenderer.draw(g,currentDesign(),resolution(),left+120,top+80,CANVAS);int[] colors=colors();for(int i=0;i<colors.length;i++){g.fill(left+120+i*16,top+231,left+135+i*16,top+247,colors[i]);if(colors[i]==brush&&!erase)g.outline(left+120+i*16,top+231,15,16,0xffffffff);}
            ...
            if(behavior)GuiText.text(g,font,"Inventory",left+278,top+60,0xa5b2dd,false);
            if(!behavior||draft.mode()!=RuneLayer.Mode.FILTER)GuiText.text(g,font,behavior?"Reserve":"X / Y (pixels)",left+278,top+145,0xa5b2dd,false);if(behavior&&draft.mode()!=RuneLayer.Mode.FILTER)GuiText.text(g,font,"Stock limit",left+347,top+145,0xa5b2dd,false);
            GuiText.text(g,font,behavior?"Priority":"Size / Rotation",left+278,top+183,0xa5b2dd,false);
            if(!behavior)GuiText.text(g,font,icons.size()+" item layers",left+278,top+219,0xa5b2dd,false);
        ...
        if(!error.isEmpty())GuiText.text(g,font,font.plainSubstrByWidth(error,W-20),left+10,top+H+3,0xff8899,false);
        for(int k=0;k<12&&resultOffset+k<results.size();k++){int x=left+279+(k%6)*22,y=top+78+(k/6)*19;if(mx>=x&&my>=y&&mx<x+20&&my<y+18)g.setTooltipForNextFrame(font,Component.literal(results.get(resultOffset+k).name()),mx,my);}
        if(colorPicker!=null){g.pose().pushMatrix();g.nextStratum();colorPicker.extractRenderState(g,mx,my,partial);g.pose().popMatrix();}
    }
}

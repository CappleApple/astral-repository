...
public final class InventoryProcessingAdapter implements ProcessingAdapter {
    ...
    @Override public CraftScheduler.Operation<ItemKey> start(CraftingService.NetworkAccess access, GlobalPos position, CraftPlan.Node<ItemKey> node) {
        ...
        var inputResource = level.getCapability(Capabilities.Item.BLOCK, position.pos(), rule.inputSide());
        IItemHandler input = inputResource == null ? null : IItemHandler.of(inputResource);
        var outputResource = level.getCapability(Capabilities.Item.BLOCK, position.pos(), rule.outputSide());
        IItemHandler output = outputResource == null ? null : IItemHandler.of(outputResource);

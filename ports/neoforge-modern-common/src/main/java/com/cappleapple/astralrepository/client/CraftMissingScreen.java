...
public final class CraftMissingScreen extends Screen {
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float partial){
        ...
        AstralInterfaceRenderer.blit(g,Identifier.fromNamespaceAndPath("astral_repository","textures/gui/rune_settings.png"),left,top,280,212);
        GuiText.text(g,font,title,left+12,top+10,0xD9DEFB,false);
        if(entries.isEmpty()){
            boolean calculating=menu.jobs.stream().anyMatch(j->j.id().equals(id)&&j.state()==NetworkPackets.JobState.CALCULATING);
            if(!calculating){parent.closeMissingDetails();return;}GuiText.text(g,font,"Calculating",left+12,top+36,0xB9B7ED,false);
        }
        for(int i=0;i<40&&row*8+i<entries.size();i++){
            ...
            RuneUi.slot(g,x,y,28,hover);g.item(missing.icon(),x+6,y+4);g.itemDecorations(font,missing.icon(),x+6,y+4,Long.toString(missing.count()));
            if(!missing.tag().isEmpty())GuiText.text(g,font,"#",x+2,y+2,0xBDB6F2,false);
            if(hover)g.setComponentTooltipForNextFrame(font,List.of(Component.literal(missing.tag().isEmpty()?missing.icon().getHoverName().getString():"#"+missing.tag()),Component.literal(Long.toString(missing.count()))),mx,my);
        }
        GuiText.text(g,font,"Esc",left+245,top+194,0xBDB6F2,false);
    }
}

...
public final class CrystalModelRenderer {
    ...
    public static net.minecraft.resources.ResourceLocation modelLocation(Block block) {
        var id = BuiltInRegistries.BLOCK.getKey(block);
        return com.cappleapple.astralrepository.platform.ClientBackport.standalone(new ResourceLocation(id.getNamespace(), "item/" + id.getPath() + "_geometry"));
    }

    public static net.minecraft.resources.ResourceLocation modelLocation(String name){return com.cappleapple.astralrepository.platform.ClientBackport.standalone(new ResourceLocation("astral_repository","item/"+name+"_geometry"));}
    ...
    public static int channel(ItemStack stack) {
        ...
        var tag = data.copy();
        return tag.contains("Channel", Tag.TAG_INT) ? com.cappleapple.astralrepository.platform.Backport.clamp(tag.getInt("Channel"), -1, 15) : -1;
    }
    ...
    public static void render(PoseStack poses, MultiBufferSource buffers, @Nullable BlockState state,
        ...
        int color = channel < 0 ? 0x65D6CF : com.cappleapple.astralrepository.platform.Backport.dyeColor(DyeColor.byId(channel));
        ...
        for (int face = 0; face <= DIRECTIONS.length; face++) {
            ...
            for (var quad : model.getQuads(state, direction, random)) {
                ...
                buffers.getBuffer(materialFor(quad)).putBulkData(poses.last(), quad, BRIGHTNESS,
                        accent ? red : 1, accent ? green : 1, accent ? blue : 1, lightmap, overlay, true);
            }
        }
    }

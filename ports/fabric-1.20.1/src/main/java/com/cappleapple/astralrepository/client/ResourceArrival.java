...
record ResourceArrival(BlockPos destination,Vec3 endpoint,Vec3 displacement,double finalLegStart) {
    static ResourceArrival create(NetworkPackets.Visual packet,long seed,double variance){
        ...
        double radius=Math.cbrt(random.nextDouble())*com.cappleapple.astralrepository.platform.Backport.clamp(variance,0,.45),horizontal=Math.sqrt(1-z*z);
        ...
        double lastLeg=TransferVisuals.legTicks(path.get(path.size()-2),path.get(path.size()-1));
    ...
    Vec3 position(Vec3 originalSplinePoint,double progress){
        double t=com.cappleapple.astralrepository.platform.Backport.clamp((progress-finalLegStart)/(1-finalLegStart),0,1);
    ...
    float opacity(Vec3 position){
        ...
        double distance=com.cappleapple.astralrepository.platform.Backport.clamp(Math.max(x,Math.max(y,z)),0,1);

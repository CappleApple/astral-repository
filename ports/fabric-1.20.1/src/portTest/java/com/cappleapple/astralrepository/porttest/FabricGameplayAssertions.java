...
final class FabricGameplayAssertions {
    ...
    static void prepare(ServerPlayer player){
        ...
        var iron=new ItemStack(Items.IRON_INGOT,512);iron.setHoverName(Component.literal("Saved identity"));stored.inventory().insertItem(0,iron,false);
        ...
        check(restored.inventory().getStackInSlot(0).getCount()==512&&ItemStack.isSameItemSameTags(iron,restored.inventory().getStackInSlot(0)),"Break/re-place retains large named stack");
        ...
        var named=new ItemStack(Items.COPPER_INGOT,32);named.setHoverName(Component.literal("Rune identity"));((Container)level.getBlockEntity(SOURCE)).setItem(0,named);
    ...
    static void verifyTransfer(ServerPlayer player){
        ...
        check(from.getCount()==20&&to.getCount()==12&&ItemStack.isSameItemSameTags(from,to),"Rune conserves identity and respects destination stock limit: "+from+" -> "+to);

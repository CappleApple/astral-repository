...
public final class RuneTransitGameTests {
    ...
    public static void noninstantPushHoldsAllFourResourcesUntilTheirArrival(PortGameTestHelper h) {
        delayed(h, RuneLayer.Mode.PUSH);
    }

    public static void noninstantPullHoldsAllFourResourcesUntilTheirArrival(PortGameTestHelper h) {
        delayed(h, RuneLayer.Mode.PULL);
    }

    private static void delayed(PortGameTestHelper h, RuneLayer.Mode mode) {
    ...
    public static void instantPushAndPullCommitAllFourResourcesOnDispatch(PortGameTestHelper h) {
        boolean automatic = AstralConfig.instantAutomaticLogistics.get();
        boolean player = AstralConfig.instantPlayerInteractions.get();
        try {
            ...
            for (var mode : List.of(RuneLayer.Mode.PUSH, RuneLayer.Mode.PULL)) {
                ...
                try {
                    ...
                    fixture.assertDestination(1, "Instant automatic mode inserts every medium in the same tick, mode="+mode+", config="+AstralConfig.instantAutomaticLogistics.get()+", pending="+RuneTransitData.get(h.getLevel().getServer()).pendingAmount(fixture.rune.id(),key(RuneCadence.Kind.ITEMS)));
                    h.assertTrue(fixture.rune.transferredItems()==BATCH[0]&&fixture.rune.transferredFluid()==BATCH[1],
    ...
    public static void oneTickCadencePipelinesThreeBatchesWithoutFinishingThemEarly(PortGameTestHelper h) {
        boolean automatic = AstralConfig.instantAutomaticLogistics.get();
        Fixture fixture = null;
        try {
            AstralConfig.instantAutomaticLogistics.set(false);
    ...
    public static void stockTargetsCountAllInFlightReservationsBeforeSchedulingMore(PortGameTestHelper h) {
        boolean automatic=AstralConfig.instantAutomaticLogistics.get();
        Fixture fixture=null;
        try {
            AstralConfig.instantAutomaticLogistics.set(false);
    ...
    public static void multipleRunesShareDestinationStockReservationsForEveryMedium(PortGameTestHelper h) {
        boolean automatic=AstralConfig.instantAutomaticLogistics.get();
    ...
    public static void fullDestinationRefundsAllFourOwnedBatchesAtArrival(PortGameTestHelper h) {
        boolean automatic=AstralConfig.instantAutomaticLogistics.get();
        Fixture fixture=null;
        try {
            AstralConfig.instantAutomaticLogistics.set(false);
            ...
            fixture.destination.tank().fill(new FluidStack(Fluids.LAVA,fixture.destination.tank().getCapacity()),IFluidHandler.FluidAction.EXECUTE);
    ...
    public static void removedDestinationRefundsOwnedBatchesWithoutUsingStaleCapabilities(PortGameTestHelper h) {
        boolean automatic=AstralConfig.instantAutomaticLogistics.get();
        Fixture fixture=null;
        try {
            AstralConfig.instantAutomaticLogistics.set(false);
    ...
    public static void savedFlightsPreserveEveryMediumAndRemainingTimeAcrossClockReset(PortGameTestHelper h) {
        ...
        var restored=RuneTransitData.load(before.save(new CompoundTag()),h.getLevel().registryAccess());
    ...
    public static void uncertainArrivalIsRecordedWithoutRefundOrReplay(PortGameTestHelper h) {
        boolean automatic=AstralConfig.instantAutomaticLogistics.get();
        RuneLayer rune=null;
        try {
            AstralConfig.instantAutomaticLogistics.set(false);
            ...
            var entries=TransferRecoveryData.get(h.getLevel().getServer()).save(new CompoundTag()).getList("Transfers",net.minecraft.nbt.Tag.TAG_COMPOUND);
            boolean found=false;
            for(int i=0;i<entries.size();i++){var entry=entries.getCompound(i);if(entry.getString("Provider").contains(id)&&entry.getBoolean("Uncertain")&&entry.getLong("Amount")==7)found=true;}
    ...
    private static final class Fixture {
        final PortGameTestHelper helper;
        ...
        Fixture(PortGameTestHelper h, RuneLayer.Mode mode, int batches) {
            helper=h;
            ...
            CompatibilityRegistry.registerResources(id,(level,pos,side)->{
                ...
                var delegate=FabricRuneTransitSmoke.scalar(id+index,arsSource,index);

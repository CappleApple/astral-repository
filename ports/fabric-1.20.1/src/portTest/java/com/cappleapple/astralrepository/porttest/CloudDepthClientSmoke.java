...
public final class CloudDepthClientSmoke {
    ...
    public static void verify() throws Exception {
        ...
        try {
            ...
            RenderSystem.getModelViewStack().popPose();RenderSystem.applyModelViewMatrix();
            ...
            if(kind==4) {
                for(int pass=0;pass<(Minecraft.useShaderTransparency()?2:1);pass++) {
                    ...
                    for(float[] xy:new float[][]{{x-.2F,-.2F},{x+.2F,-.2F},{x+.2F,.2F},{x-.2F,.2F}})
                        out.vertex(xy[0],xy[1],z).color(255,190,40,210).endVertex();
                ...
                var packet=new NetworkPackets.Visual(BlockPos.ZERO,new BlockPos(1,0,0),ItemStack.EMPTY,0xffffff,20,slot,
                    java.util.List.of(BlockPos.ZERO,new BlockPos(1,0,0)),null,null,new ResourceLocation(medium==1?"lava":"water"));
    ...
    private static void solid(float left,float right,float z,int alpha) {
        ...
        var out=Tesselator.getInstance().getBuilder();out.begin(VertexFormat.Mode.QUADS,DefaultVertexFormat.POSITION_COLOR);
        for(float[] xy:new float[][]{{left,-1},{right,-1},{right,1},{left,1}})out.vertex(xy[0],xy[1],z).color(160,160,160,alpha).endVertex();
        BufferUploader.drawWithShader(out.end());
    }

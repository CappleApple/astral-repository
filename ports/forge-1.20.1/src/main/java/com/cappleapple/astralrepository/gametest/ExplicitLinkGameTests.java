...
public final class ExplicitLinkGameTests {
    ...
    public static void runeUnloadPreservesLinksAndRebuildCleansReplacedHosts(GameTestHelper h){
        ...
        var rune=RuneSurfaces.getOrCreate(h.getLevel(),h.absolutePos(hostPos),Direction.NORTH);rune.setChannel(5);rune.addGlyph(new ResourceLocation("astral_repository:filter_sigil"),NodeKind.ROUTING);
        ...
        h.succeedWhen(()->{
            ...
            try{
                ...
                NetworkManager.chunkUnload(new net.minecraftforge.event.level.ChunkEvent.Unload(chunk));
                h.setBlock(hostPos,Blocks.BARREL);
                NetworkManager.chunkLoad(new net.minecraftforge.event.level.ChunkEvent.Load(chunk,false));
    ...
    public static void separateLinkedFacesKeepIndependentNetworkAccessWithoutDuplicatingTransfers(GameTestHelper h){
        ...
        for(var rune:List.of(north,south)){rune.setChannel(7);rune.addGlyph(new ResourceLocation("astral_repository:filter_sigil"),NodeKind.ROUTING);}
        south.setChannel(8);
    ...
    public static void runeFacesStackRulesPersistLinksAndUseTheActualSide(GameTestHelper h){
        ...
        destination.setChannel(9);destination.addGlyph(new ResourceLocation("astral_repository:routing_rune"),NodeKind.ROUTING);
        destination.addGlyph(new ResourceLocation("astral_repository:filter_sigil"),NodeKind.ROUTING);destination.addGlyph(new ResourceLocation("astral_repository:filter_sigil"),NodeKind.ROUTING);
        var pull=destination.layers().get(0);pull.filter().add(FilterRules.Kind.ITEM,"minecraft:iron_ingot",false,ItemStack.EMPTY);pull.filter().setTarget(16);pull.changed();
        ...
        RuneSurface source=RuneSurfaces.getOrCreate(h.getLevel(),h.absolutePos(sourcePos),Direction.UP);source.setChannel(9);source.addGlyph(new ResourceLocation("astral_repository:collection_rune"),NodeKind.COLLECTION);

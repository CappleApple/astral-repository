...
public final class RuneSettingsPackets {
    ...
    private static boolean nearby(ServerPlayer player,RuneSurface surface){return player.isAlive()&&player.serverLevel()==surface.getLevel()&&!surface.isRemoved()&&player.distanceToSqr(surface.getBlockPos().getCenter())<=Math.pow(player.getBlockReach()+1,2);}
    ...
    public static String sampleRule(ItemStack sample){
        var fluid=net.minecraftforge.fluids.FluidUtil.getFluidContained(sample);

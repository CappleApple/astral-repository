    ...
    public static void networkRunesUseActualStorageTravelTimesAndKeepPolicyAcrossRebuilds(GameTestHelper h)throws Exception {
        ...
        try {
            ...
            for(var mode:List.of(RuneLayer.Mode.PUSH,RuneLayer.Mode.PULL)) {
                ...
                BlockPos hostPos=new BlockPos(1,25,z),relayPos=new BlockPos(4,25,z),nearPos=new BlockPos(18,25,z),farPos=new BlockPos(32,25,z);
                for(var pos:List.of(hostPos,relayPos,nearPos,farPos))h.getLevel().getChunkAt(h.absolutePos(pos));
                // The long route extends beyond the empty structure into generated terrain.
                for(int x=1;x<=32;x++)h.setBlock(new BlockPos(x,25,z),Blocks.AIR);
                ...
                h.assertTrue(network!=null&&manager.networkAt(near.address())==network&&manager.networkAt(far.address())==network,"Fixture has one connected storage component: relay="+network+", near="+manager.networkAt(near.address())+", far="+manager.networkAt(far.address())+", sight="+LineOfSight.clear(h.getLevel(),relay.getBlockPos(),near.getBlockPos(),p->{})+"/"+LineOfSight.clear(h.getLevel(),near.getBlockPos(),far.getBlockPos(),p->{})+", mode="+mode+"");
                ...
                var restored=RuneTransitData.load(RuneTransitData.get(h.getLevel().getServer()).save(new CompoundTag()),h.getLevel().registryAccess());

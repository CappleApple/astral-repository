...
public final class RuneTransitData extends SavedData {
    ...
    public static RuneTransitData get(MinecraftServer server){return server.overworld().getDataStorage().computeIfAbsent(new SavedData.Factory<>(RuneTransitData::new,RuneTransitData::load,null),"astral_repository_rune_transit");}
    ...
    public static RuneTransitData load(CompoundTag tag,HolderLookup.Provider registries){
        ...
        for(int i=0;i<entries.size();i++){var value=entries.getCompound(i);try{
            ...
            if(value.contains("Item"))resource=new ItemKey(ItemStack.parseOptional(registries,value.getCompound("Item")));
            else if(value.contains("Fluid"))resource=new FluidKey(FluidStack.parseOptional(registries,value.getCompound("Fluid")));
            else resource=new ResourceKinds.ScalarKey(ResourceLocation.parse(value.getString("Type")),ResourceLocation.parse(value.getString("Resource")));
    ...
    private static Endpoint readEndpoint(CompoundTag tag){
        var pos=GlobalPos.of(net.minecraft.resources.ResourceKey.create(Registries.DIMENSION,ResourceLocation.parse(tag.getString("Dimension"))),BlockPos.of(tag.getLong("Position")));
    ...
    @Override public CompoundTag save(CompoundTag tag,HolderLookup.Provider registries){
        ...
        for(var bucket:due.entrySet())for(var flight:bucket.getValue()){
            ...
            if(flight.resource instanceof ItemKey item)value.put("Item",item.sample().save(registries));
            else if(flight.resource instanceof FluidKey fluid)value.put("Fluid",fluid.sample().save(registries));

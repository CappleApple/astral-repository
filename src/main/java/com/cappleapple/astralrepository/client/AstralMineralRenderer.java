...
public final class AstralMineralRenderer implements BlockEntityRenderer<AstralMineralBlockEntity> {
    ...
    @Override public void render(AstralMineralBlockEntity entity, float partialTick, PoseStack poses, MultiBufferSource buffers, int light, int overlay) {
        ...
        for (Direction side : Direction.values()) {
            ...
            random.setSeed(42);
            for (var quad : model.getQuads(state, side, random)) consumer.putBulkData(poses.last(), quad, 1, 1, 1, light, overlay);
        }
        random.setSeed(42);
        for (var quad : model.getQuads(state, null, random)) consumer.putBulkData(poses.last(), quad, 1, 1, 1, light, overlay);
    }
}

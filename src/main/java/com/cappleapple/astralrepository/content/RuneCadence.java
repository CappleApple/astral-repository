...
public record RuneCadence(Map<Kind,Rate> overrides) {
    ...
    public enum Kind {
        ...
        public final ResourceLocation resource; public final String title;
        Kind(ResourceLocation resource,String title){this.resource=resource;this.title=title;}
        ...
        public static Kind of(ResourceLocation resource){for(var kind:values())if(kind.resource.equals(resource))return kind;throw new IllegalArgumentException("Unsupported resource kind: "+resource);}
    }
    ...
    public static RuneCadence load(CompoundTag tag){var rates=new EnumMap<Kind,Rate>(Kind.class);for(var kind:Kind.values())if(tag.contains(kind.name())){var v=tag.getCompound(kind.name());rates.put(kind,new Rate(v.getInt("Amount"),v.getInt("Ticks")));}return new RuneCadence(rates);}
}

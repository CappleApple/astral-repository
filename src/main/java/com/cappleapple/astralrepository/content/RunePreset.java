...
public record RunePreset(UUID id,String name,RuneLayer.Mode mode,RuneDesign design,CompoundTag filter,int priority,boolean enabled,RuneCadence cadence) {
    ...
    public CompoundTag save(){CompoundTag t=new CompoundTag();t.put("Cadence",cadence.save());t.store("Id",net.minecraft.core.UUIDUtil.CODEC,id);t.putString("Name",name);t.putString("Mode",mode.name());t.put("Design",design.save());t.put("Filter",filter.copy());t.putInt("Priority",priority);t.putBoolean("Enabled",enabled);return t;}
    public static RunePreset load(CompoundTag t,HolderLookup.Provider registries){FilterRules rules=new FilterRules();rules.load(t.getCompoundOrEmpty("Filter"),registries);return new RunePreset(t.read("Id",net.minecraft.core.UUIDUtil.CODEC).orElseThrow(),t.getStringOr("Name",""),RuneLayer.Mode.valueOf(t.getStringOr("Mode","")),RuneDesign.load(t.getCompoundOrEmpty("Design")),rules.save(registries),t.getIntOr("Priority",0),!t.contains("Enabled")||t.getBooleanOr("Enabled",false),RuneCadence.load(t.getCompoundOrEmpty("Cadence")));}

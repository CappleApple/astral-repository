...
public final class StorageCrystalClientSmoke {
    ...
    public static boolean tick()throws Exception{
        ...
        if(phase==0){phase=1;mc.getSingleplayerServer().execute(()->{try{
            var p=mc.getSingleplayerServer().getPlayerList().getPlayers().get(0);var level=p.serverLevel();p.closeContainer();level.getChunkAt(CRYSTAL);
            ...
            check(screen.getMenu().entries.size()==1&&screen.getMenu().entries.get(0).stack().is(Items.AMETHYST_SHARD)&&screen.getMenu().entries.get(0).count()==23,"Crystal received foreign network items");
    ...
    private static void open(BlockPos pos){var mc=Minecraft.getInstance();mc.getSingleplayerServer().execute(()->{try{var p=mc.getSingleplayerServer().getPlayerList().getPlayers().get(0);NetworkManager.open(p,GlobalPos.of(p.level().dimension(),pos));}catch(Throwable e){failure=e.toString();}});}

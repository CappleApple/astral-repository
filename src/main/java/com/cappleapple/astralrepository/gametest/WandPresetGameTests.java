...
public final class WandPresetGameTests {
    ...
    public static void pushAndPullUseWholeNetworkWithoutMovingBackIntoTheirOwnHost(GameTestHelper h){
        ...
        var activeLayers=new ArrayList<RuneLayer>();activeLayers.add(push);
        var iron=new com.cappleapple.astralrepository.api.ItemKey(new ItemStack(Items.IRON_INGOT));
        h.onEachTick(()->{
            var transit=RuneTransitData.get(h.getLevel().getServer());long total=count(a)+count(b)+count(c);
            for(var layer:activeLayers)total+=transit.pendingAmount(layer.id(),iron);
            h.assertTrue(total==40,"Physical storage plus in-flight ownership conserves the source items every tick");
        });
        h.startSequence().thenWaitUntil(()->h.assertTrue(count(b)==40&&count(a)==0,"Push filled another network store and did not reinsert into its own indexed host")).thenExecute(()->{push.setEnabled(false);var destination=RuneSurfaces.getOrCreate(h.getLevel(),h.absolutePos(out),Direction.SOUTH);var pull=destination.addLayer(com.cappleapple.astralrepository.content.RuneGlyph.id(com.cappleapple.astralrepository.content.RuneLayer.Mode.PULL),RuneLayer.Mode.PULL);activeLayers.add(pull);pull.filter().add(FilterRules.Kind.ITEM,"minecraft:iron_ingot",false,ItemStack.EMPTY);pull.filter().setTarget(24);h.assertTrue(destination.toggleTarget(pull.id(),GlobalPos.of(h.getLevel().dimension(),h.absolutePos(nexus)),null).success(),"Pull binds a network");}).thenWaitUntil(()->h.assertTrue(count(c)==24&&count(b)==16,"Pull stocks to its target from the network: source="+count(a)+", store="+count(b)+", output="+count(c)+", rune="+RuneSurfaces.get(h.getLevel(),h.absolutePos(out),Direction.SOUTH).layers().getFirst().status())).thenSucceed();
    }

...
public final class WandPresetGameTests {
    ...
    public static void paintedPresetPlacementPersistsIndependentCopiesAndNearestFreePositions(GameTestHelper h)throws Exception {
        ...
        library.remove(preset.id());var loaded=RuneSurface.load(h.getLevel().getServer(),surface.save(h.getLevel().registryAccess()),h.getLevel().registryAccess());h.assertTrue(loaded.layers().size()==4&&loaded.layers().get(0).design().pixel(63,63)==45&&loaded.layers().get(0).design().icons().equals(List.of(icon)),"Placed design survives deletion and reload without flattening item layers");h.assertTrue(loaded.layers().get(0).filter().target()==32&&loaded.layers().get(0).priority()==7,"Placed behavior survives deletion and reload");
        var buffer=new net.minecraft.network.FriendlyByteBuf(io.netty.buffer.Unpooled.buffer(),h.getLevel().registryAccess());try{var action=new WandPackets.Action(UUID.randomUUID(),WandPackets.Op.IMPORT,preset.id(),preset.save());WandPackets.Action.CODEC.encode(buffer,action);h.assertTrue(WandPackets.Action.CODEC.decode(buffer).data().equals(action.data()),"Actual bounded editor codec retains the preset");}finally{buffer.release();}h.succeed();
    }
    ...
    public static void wandRelayTargetPushesAndPullsAcrossConnectedNetwork(GameTestHelper h){
        ...
        player.gameMode.useItemOn(player,h.getLevel(),wand,InteractionHand.MAIN_HAND,new BlockHitResult(RuneLayout.placed(face).get(0).center(),Direction.SOUTH,h.absolutePos(source),false));
    ...
    public static void networkRuneRefundsReturnItemsAndFluidsWithoutReplay(GameTestHelper h){
        ...
        recovery.put(address,new com.cappleapple.astralrepository.api.FluidKey(new net.minecraftforge.fluids.FluidStack(net.minecraft.world.level.material.Fluids.WATER,1)),1000,false,provider+"_fluid",null);

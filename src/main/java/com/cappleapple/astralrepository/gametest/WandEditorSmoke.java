...
public final class WandEditorSmoke {
    ...
    public static boolean tick()throws Exception {
        ...
        if(phase==0){phase=1;mc.getSingleplayerServer().execute(()->{try{var p=mc.getSingleplayerServer().getPlayerList().getPlayers().getFirst();p.getInventory().selected=0;p.setItemInHand(InteractionHand.MAIN_HAND,new ItemStack(AstralContent.ATTUNEMENT_WAND.get()));WandPackets.open(p);}catch(Throwable t){failure=t.toString();}});}
            ...
            int[] noise=new int[128*128];for(int i=0;i<noise.length;i++)noise[i]=0xff000000|(i*7919)&0xffffff;fullColorId=UUID.randomUUID();var large=new RunePreset(fullColorId,"128 RGB transport",RuneLayer.Mode.FILTER,new RuneDesign(128,noise,List.of()),saved.filter(),0,true);var field=WandScreen.class.getDeclaredField("session");field.setAccessible(true);net.neoforged.neoforge.network.PacketDistributor.sendToServer(new WandPackets.Action((UUID)field.get(screen),WandPackets.Op.IMPORT,fullColorId,large.save()));com.cappleapple.astralrepository.AstralServerConfig.runeResolution.set(16);phase=5;ticks=0;}

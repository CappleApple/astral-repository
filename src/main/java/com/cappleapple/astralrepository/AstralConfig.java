...
public final class AstralConfig {
    ...
    public static final ModConfigSpec.BooleanValue instantAutomaticLogistics = B.comment("Run automatic routing and default rune intervals every tick, deliver automatic rune batches immediately, and skip crafting delivery waits and Astral-managed table/stonecutter delays. Explicit rune intervals, quantity limits, power costs, and physical or external machine processing remain unchanged. When false, rune batches leave at cadence and enter their destination after their route travel time.").define("instantAutomaticLogistics", false);

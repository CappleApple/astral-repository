...
public final class VanillaProcessingAdapter implements ProcessingAdapter {
    ...
    static String ingredientTag(Ingredient ingredient) {
        ...
        try { return ingredient.display() instanceof net.minecraft.world.item.crafting.display.SlotDisplay.TagSlotDisplay tag ? tag.tag().unwrapKey().map(key -> key.location().toString()).orElse("") : ""; }

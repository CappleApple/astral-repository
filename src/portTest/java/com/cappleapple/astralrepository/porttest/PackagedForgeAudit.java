...
public final class PackagedForgeAudit {
    ...
    private static final Path STATE=Path.of("packaged-audit-state.txt"),RESULT=Path.of("packaged-audit-result.txt");
    ...
    private void tick(TickEvent.ServerTickEvent event){
        ...
        try {
            ...
            if(restart){
                ...
                finish("PASS: installed Forge 47.4.10; actual release JAR; ten real block-item placements; reobfuscated NetworkAnchor dispatch; linked Nexus withdrawal/deposit and vanilla crafting/refill; real scheduled rune transfer; second-process disk persistence of stock, NBT identity, upgrades, fluids, energy and runes.");return;
            }
            ...
            Files.writeString(STATE,"First process passed; awaiting restart.\n");finish("FIRST_PASS: packaged gameplay complete; restart required for disk persistence.");
        }catch(Throwable failure){fail(failure);}
    }

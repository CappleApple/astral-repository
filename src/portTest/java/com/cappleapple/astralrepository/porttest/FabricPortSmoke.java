...
public final class FabricPortSmoke implements ModInitializer {
 public void onInitialize(){if(!Boolean.getBoolean("astral_repository.portSmoke"))return;ServerLifecycleEvents.SERVER_STARTED.register(server->{try{
  ...
  var saved=node.saveWithFullMetadata(world.registryAccess());var restored=(CrystalNodeBlockEntity)net.minecraft.world.level.block.entity.BlockEntity.loadStatic(pos,node.getBlockState(),saved,world.registryAccess());check(restored!=null,"block entity restore");restored.setLevel(world);check(restored.inventory().getStackInSlot(0).getCount()==23,"item persistence");check(restored.tank().getFluidAmount()==1000,"fluid persistence");
  ...
  var drops=net.minecraft.world.level.block.Block.getDrops(cluster,world,pos,null,null,pickaxe);check(drops.size()==1&&drops.getFirst().is(AstralContent.ASTRAL_GEM.get())&&drops.getFirst().getCount()==4,"cluster pickaxe drops four gems: "+drops);
  drops=net.minecraft.world.level.block.Block.getDrops(cluster,world,pos,null,null,net.minecraft.world.item.ItemStack.EMPTY);check(drops.size()==1&&drops.getFirst().is(AstralContent.ASTRAL_GEM.get())&&drops.getFirst().getCount()==2,"cluster hand drops two gems: "+drops);
  var silk=pickaxe.copy();silk.enchant(world.registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(net.minecraft.world.item.enchantment.Enchantments.SILK_TOUCH),1);

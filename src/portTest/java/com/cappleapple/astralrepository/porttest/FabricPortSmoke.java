...
public final class FabricPortSmoke implements ModInitializer {
 public void onInitialize(){if(!Boolean.getBoolean("astral_repository.portSmoke"))return;ServerLifecycleEvents.SERVER_STARTED.register(server->{try{
  ...
  var saved=node.saveWithFullMetadata(world.registryAccess());var restored=new CrystalNodeBlockEntity(pos,node.getBlockState());restored.setLevel(world);restored.loadWithComponents(saved,world.registryAccess());check(restored.inventory().getStackInSlot(0).getCount()==23,"item persistence");check(restored.tank().getFluidAmount()==1000,"fluid persistence");
  var recipeIds=server.getRecipeManager().getRecipes().stream().map(recipe->recipe.id()).filter(id->id.getNamespace().equals("astral_repository")).map(net.minecraft.resources.ResourceLocation::getPath).collect(java.util.stream.Collectors.toSet());

# Astral Repository â€” Fabric 1.20.1

A magical storage and automation network for Minecraft 1.20.1. Crystals connect nearby storage, relay resources, and coordinate crafting through ordinary workstations. The `CappleApple/fabric-1.20.1` branch contains Astral Repository 1.11.8 for Fabric.

## Installation

Install `astral_repository-fabric-1.20.1-1.11.8.jar` on both the client and server. Requires Minecraft **1.20.1**, **Fabric Loader 0.19.5 or newer**, **Fabric API 0.92.12+1.20.1**, and **Java 17**. Required configuration, energy, and applicable model libraries are bundled in the mod JAR.

Optional Trinkets provides the goggles accessory slot; the normal helmet slot works without it. Patchouli enables the Field Guide. AE2 Fabric 1.20.1 has an explicit storage adapter; full optional-mod networks were not part of the runtime checks.
...
Run Gradle with **JDK 21**. An installed JDK 17 is also required for compilation. If Gradle cannot discover it, set `org.gradle.java.installations.paths` to its location in your user Gradle properties.

```powershell
.\gradlew.bat test build
```

On Linux or macOS, use `./gradlew test build`. The output is `build/libs/astral_repository-fabric-1.20.1-1.11.8.jar`. Do not package a release with `-PclientSmoke`.

...
A magical storage and automation network for Minecraft 1.20.1. Crystals connect nearby storage, relay resources, and coordinate crafting through ordinary workstations. This is Astral Repository **1.11.11**, on branch `CappleApple/fabric-1.20.1`.

## Installation

Install `astral_repository-fabric-1.20.1-1.11.11.jar` on the client and server. Requires **Minecraft 1.20.1**, **Fabric Loader 0.19.5 or newer**, **Fabric API 0.92.12+1.20.1**, and **Java 17**. Configuration, energy, and applicable model libraries are bundled in the mod JAR.
...
On Linux or macOS, use `./gradlew test build`. The output is `build/libs/astral_repository-fabric-1.20.1-1.11.11.jar`. All source sets and resources are inside this branch; no sibling checkout is required.

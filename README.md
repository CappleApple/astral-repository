...
A magical storage and automation network for Minecraft 26.2. Crystals connect nearby storage, relay resources, and coordinate crafting through ordinary workstations. This branch contains Astral Repository 1.11.11 for Fabric.

## Installation

Install `astral_repository-fabric-26.2-1.11.11.jar` on both the client and server. Requires Minecraft **26.2**, **Fabric Loader 0.19.5 or newer**, **Fabric API 0.161.0+26.2**, and **Java 25**. Configuration and energy libraries are bundled in the mod JAR.
...
On Linux or macOS, use `./gradlew test build`. The output is `build/libs/astral_repository-fabric-26.2-1.11.11.jar`. Release builds must omit `-PclientSmoke`.

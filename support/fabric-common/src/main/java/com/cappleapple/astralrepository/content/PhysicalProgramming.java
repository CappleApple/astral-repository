...
public final class PhysicalProgramming {
    ...
    public static boolean use(ItemStack stack,Level level,BlockPos pos,Player player,InteractionHand hand,BlockHitResult hit) {
        ...
        if(stack.getItem() instanceof DyeItem dye)node.setChannel(stack.get(net.minecraft.core.component.DataComponents.DYE).getId());
    ...
    public static boolean hasRemoteAttunement(ItemStack stack){return stack.getOrDefault(DataComponents.CUSTOM_DATA,CustomData.EMPTY).copyTag().getBooleanOr("DimensionalRemote",false);}

    ...
    public static boolean tick()throws Exception{
        ...
        if(phase==0){phase=1;ticks=0;NetworkPackets.visualReceiver=p->{WorldVisuals.add(p);if(p.stack().is(Items.LAPIS_LAZULI))runeFlight=p;if(p.slot()==-2||p.slot()==-3)resourceFlights.add(p);if(p.stack().is(Items.EMERALD)){received=p;started=mc.level.getGameTime();}if(p.slot()<0&&p.from().getX()>=20&&(p.stack().is(Items.IRON_INGOT)||p.stack().is(Items.IRON_TRAPDOOR)))craftingFlights.add(p);};
            mc.getSingleplayerServer().execute(()->{try{
                var p=mc.getSingleplayerServer().getPlayerList().getPlayers().getFirst();var level=p.serverLevel();p.closeContainer();AstralConfig.instantPlayerInteractions.set(true);
                ...
                var p=mc.getSingleplayerServer().getPlayerList().getPlayers().getFirst();var origin=GlobalPos.of(p.level().dimension(),NEXUS);var n=NetworkManager.get(p.server).networkAt(origin);check(n!=null,"Automatic client network missing");
            ...
            var ingredient=craftingFlights.getFirst();var product=craftingFlights.getLast();
